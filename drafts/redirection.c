...
/*   Updated: 2020/11/03 12:12:45 by hmiso            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishel.h"

void		ft_redirects(char *path, char **comand, t_vars *vars)
{
    pid_t pid;
    int mas[2];
    int status;
    ...
    if (pid == 0)
    {
		...
		else if ((status = execve(path, comand_s, vars->envp_copy)) == -1)
