...
/*   Updated: 2020/11/27 21:20:51 by hmiso            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_conveyor(char *line, char **comand_line, t_vars *vars)
{
	...
	{
		if (conveyor.i != 0 && ((ft_strncmp(comand_line[conveyor.i], "|", 2) == 0)
		...
		{
			conveyor.com_whis_flags = make_comand_mas_start(comand_line, conveyor.i, (conveyor.j - 1));
			if (conveyor.com_whis_flags[0] != NULL)
				conveyor_res_3(comand_line, vars, &conveyor);
			else
				conveyor_res_4(comand_line, vars, &conveyor);
		}
		else if ((ft_strncmp(comand_line[conveyor.i], "<", 2) == 0) && vars->mas_flags[conveyor.i] != 1)
		{
			conveyor.com_whis_flags = make_comand_mas_start(comand_line, conveyor.i, (conveyor.j - 1));
			if(conveyor.com_whis_flags[0] != NULL)
			{
